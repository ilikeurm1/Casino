...
from settings import sleep, console, randint, FloatPrompt
from events import *
...
def roll_anim(slots: list[int]) -> None:
    ...
    time = 3  # Around time the animation will take in seconds
    ...
    for x in range(frames):         # For every frame
        if x == frames // 3:        # If we reached one third off the total frames
            filled[0] = True        # Start showing the first number
        elif x == 2 * frames // 3:  # When we reach two thirds
            filled[1] = True        # Show the second number

        # Print the correct slot machine
        console.print(
            machine.format(
                slots[0] if filled[0] else randint(1, 9),
                slots[1] if filled[1] else randint(1, 9),
                randint(1, 9),
            ),
            style="b magenta",
        )
        ...
        )  # Move up `slot_machine_height` lines
        sleep(
            time / frames
        )  # Sleep for the correct amount of time to reach a total of ~`time` seconds in the end

    # Print the full machine when done so in the end the player looks at the slots list
...
def run_random_events(user: str, money: int) -> int:
    console.print()
    run_hobo = randint(1, 100)
    if run_hobo < 15: # 15% chance
        return drunk_hobo(money)
    
    run_random_money = randint(1, 100)
    if run_random_money < 15: # 15% chance
        return random_money(money)
...
def DEBUG_GAME(DEBUG_MODE: int, game: str, data: dict[str, Any]) -> tuple:
    ...
    if game == "guesser":
        ...
        vals = (
            (data["chosen"], data["winning_number"])
            if DEBUG_MODE == 1
            else (
                (69, 69)
                if DEBUG_MODE == 2
        ...
        vals = (
            (  # Don't change the values if the user is playtesting
                data["chosen_number"],
                data["chosen_color"],
                data["winning_number"],
                data["winning_color"],
            ...
            if DEBUG_MODE == 1
                ...
                (  # Set both the chosen and winning number to 0 so the player always wins
                    0,
                    "g",
                    0,
                    "g",
                ...
                if DEBUG_MODE == 2
                else (  # Debug mode 3 if it comes in the future
                    data["chosen_number"],
                    data["chosen_color"],
                    data["winning_number"],
                    data["winning_color"],
        ...
        vals = (
            (data["slot_machine"])
            if DEBUG_MODE == 1
            else (7, 7, 7) if DEBUG_MODE == 2 else (data["slot_machine"])
        )
        ...
        vals = (
            (data["player_hand"], data["dealer_hand"])
            if DEBUG_MODE == 1
            else (
                ([10, 11], [10, 10, 2])
                if DEBUG_MODE == 2
        ...
        console.print(
            f"[blue]DEBUG PRINT: Player's hand was: {data["player_cards"]}, Banker's hand was: {data["banker_cards"]}"
        )

        vals = (
            (data["player_cards"], data["banker_cards"])
            if DEBUG_MODE == 1
            else (
                ([8, 1], [6])
                if DEBUG_MODE == 2
                else (data["player_cards"], data["banker_cards"])
            )
        )
